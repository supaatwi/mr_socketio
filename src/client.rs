...
impl SocketIOClient {
    ...
    // Asynchronous method to register event handlers
    ...
    pub async fn connect(&self) -> Result<(), SocketIOError> {
        let handshake_url = format!("{}/socket.io/?EIO=4&transport=polling", self.base_url);
        ...
        let ws_url = format!(
            "ws{}://{}/socket.io/?EIO=4&transport=websocket&sid={}",
        ...
        while let Some(msg) = read.next().await {
            match msg? {
                Message::Text(text) => {
                    ...
                    } else if upgraded && text.starts_with("42") {
